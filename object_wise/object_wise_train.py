...
for epoch in range(opt.nepoch):
    for i, data in enumerate(tqdm(dataloader), 0):
        ...
        pos_loss = F.mse_loss(pred_pos.float(), pos.float()) * 10
        cos_loss = F.mse_loss(pred_cos.float(), cos.float()) * 6
        sin_loss = F.mse_loss(pred_sin.float(), sin.float()) * 6
        width_loss = F.mse_loss(pred_width.float(), width.float()) * 8
