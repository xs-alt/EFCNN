#!/usr/bin/env python
# -*- coding: utf-8 -*-

import torch
import torch.utils.data as data
from sklearn.model_selection import KFold

import os
import numpy as np
import pickle
import sys
import gc
from tqdm import tqdm
from collections import defaultdict


class Cornell(data.Dataset):
    def __init__(self, root, train=True, kf_num=None):
        self.data_path = root
        self.train = train
        self.files = os.listdir(self.data_path)

        raw_dict = defaultdict(list)
        with open(
                '/media/ros/0D1416760D141676/Documents/CORNELL/processedData/z.txt', 'r') as txt_file:
            lines = txt_file.read().split('\n')
        for i, item in enumerate(lines):
            item = item.split(' ')
            if len(item) == 4:
                if not item[1] in raw_dict[int(item[0])]:
                    raw_dict[int(item[0])].append(item[1])

        print('preparing dataset ...')
        self.dataset = defaultdict(list)
        for i, _ in enumerate((tqdm(self.files))):
            meta = self.files[i]
            with open(os.path.join(self.data_path, meta), 'rb') as handle:
                if sys.version_info.major > 2:
                    file = pickle.load(handle, encoding='latin1')
                else:
                    file = pickle.load(handle)
                object_id = file['img_id']
                object_id = int(object_id[0])
                self.dataset[raw_dict[object_id][0]].append(meta)
        print('done!')

        kf_raw_dataset = []
        kf = KFold(n_splits=5)
        kf_train_sets = []
        kf_test_sets = []
        for key, value in self.dataset.items():
            kf_raw_dataset.append({key: value})

        for train_set, test_set in kf.split(kf_raw_dataset):
            kf_train_sets.append(train_set)
            kf_test_sets.append(test_set)

        if train and kf_num is not None:
            train_files = []
            for item in kf_train_sets[kf_num]:
                for file in list(kf_raw_dataset[item].values())[0]:
                    train_files.append(file)
            self.num = len(train_files)
            self.files = train_files

        if not train and kf_num is not None:
            test_files = []
            for item in kf_test_sets[kf_num]:
                for file in list(kf_raw_dataset[item].values())[0]:
                    test_files.append(file)
            self.num = len(test_files)
            self.files = test_files
        print('done')

    def __len__(self):
        return self.num

    def __getitem__(self, index):
        meta = self.files[index]

        with open(os.path.join(self.data_path, meta), 'rb') as handle:
            if sys.version_info.major > 2:
                file = pickle.load(handle, encoding='latin1')
            else:
                file = pickle.load(handle)

            rgb = np.asarray(file['rgb'])
            rgb = np.squeeze(rgb, axis=0)

            depth = np.asarray(file['depth_inpainted'])
            depth = np.squeeze(depth, axis=0)

            pos_img = np.asarray(file['grasp_points_img'])
            pos_img = np.squeeze(pos_img, axis=0)

            angle_img = np.asarray(file['angle_img'])
            angle_img = np.squeeze(angle_img, axis=0)

            width_img = np.asarray(file['grasp_width'])
            width_img = np.squeeze(width_img, axis=0)

            bbs = np.asarray(file['bounding_boxes'])
            bbs = np.squeeze(bbs, axis=0)

            rgb = torch.tensor(rgb)  # 3 400 400
            depth = torch.tensor(depth)  # 1 400 400
            grasp_points_img = torch.tensor(pos_img)
            grasp_width = torch.tensor(width_img)
            cos_img = np.cos(2 * angle_img)
            sin_img = np.sin(2 * angle_img)
            cos_img = torch.tensor(cos_img)
            sin_img = torch.tensor(sin_img)

            if self.train is False:
                bbs = torch.tensor(bbs)
                return rgb, depth, grasp_points_img, cos_img, sin_img, grasp_width, bbs
            else:
                return rgb, depth, grasp_points_img, cos_img, sin_img, grasp_width


if __name__ == '__main__':
    cornell = Cornell(
        '/media/ros/0D1416760D141676/Documents/CORNELL/new',
        train=False,
        kf_num=1)
    print(len(cornell))
