...
def run(network):
    ...
    with torch.no_grad():
        ...
        for i, _ in enumerate(tqdm(cornell)):
            ...
            if VISUALIZE_FAILURES:
                ...
                for i in failed:
                    ...
                    plot_output(rgb_imgs,
                                depth_imgs[i,
                                           ],
                                grasp_positions_out[i,
                                                    ].squeeze(),
                                grasp_angles_out[i,
                                                 ].squeeze(),
                                bbs_all[i,
                                ...
                                no_grasps=NO_GRASPS,
                                grasp_width_img=grasp_width_out[i,
                                                                ].squeeze())
        ...
        print('succeed', succeed_count * 1.0 /
              (succeed_count + fail_count + 0.000001))

    return succeed_count * 1.0 / (succeed_count + fail_count + 0.000001)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vis_failures', type=bool, default=False,
    ...
    cornell = Cornell(opt.datasets, train=False, kf_num=opt.kf)
    run(opt.model)
