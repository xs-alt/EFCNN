...
from datasets_multi import Georgia
...
def run(network):
    ...
    with torch.no_grad():
        ...
        for i,_ in enumerate(tqdm(georgia)):
            rgb, depth, grasp_points_img, cos_img, sin_img, grasp_width, bbs = georgia[i]
            rgb_imgs = rgb
            depth_imgs = depth
...
if __name__ == '__main__':
    ...
    georgia = Georgia(opt.datasets, train=False)
    run(opt.model)
