...
# convert from the text to the number
...
class BoundingBoxes(object):
    ...
    def __getattr__(self, attr):
        ...
        if hasattr(BoundingBox, attr) and callable(getattr(BoundingBox, attr)):
            return lambda *args, **kwargs: list(
                map(lambda bb: getattr(bb, attr)(*args, **kwargs), self.bbs))
        else:
            raise AttributeError(
                "Couldn't find function %s in BoundingBoxes or BoundingBox" %
                attr)
    ...
    # the shape is the same as the depth image
    ...
    def to_array(self, pad_to=0):
        ...
        if pad_to:
            if pad_to > len(self.bbs):
                a = np.concatenate(
                    (a, np.zeros((pad_to - len(self.bbs), 4, 2))))
...
class BoundingBox:
    ...
    def angle(self):
        ...
        return (np.arctan2(-dy, dx) + np.pi / 2) % np.pi - \
            np.pi / 2  # between -pi/2 and pi/2
    ...
    def compact_polygon_coords(self, shape=None):
        return Grasp(self.center, self.angle, self.length / 3,
                     self.width).as_bb.polygon_coords(shape)
    ...
    def rotate(self, angle, center):
        ...
        self.points = ((np.dot(R, (self.points - c).T)).T +
                       c).astype(np.int)  # note
    ...
    def zoom(self, factor, center):
        ...
        c = np.array(center).reshape((1, 2))
        self.points = ((np.dot(T, (self.points - c).T)).T +
                       c).astype(np.int)  # note
...
def detect_grasps(
    ...
    for grasp_point_array in local_max:
        ...
        if ang_threshold > 0:
            if grasp_angle > 0:
                grasp_angle = ang_img[grasp_point[0] -
                                      ang_threshold:grasp_point[0] +
                                      ang_threshold +
                                      1, grasp_point[1] -
                                      ang_threshold:grasp_point[1] +
                                      ang_threshold +
                                      1].max()
            else:
                grasp_angle = ang_img[grasp_point[0] -
