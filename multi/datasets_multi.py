...
# make all the images evaluation set
TRAIN_SPLIT = 0


class Georgia(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(self.data_path, meta), 'rb') as handle:
            ...
            ang_img = np.asarray(file['angle_img'])
            ang_img = np.squeeze(ang_img, axis=0)
...
if __name__ == '__main__':
    georgia = Georgia(
        '/media/ros/0D1416760D141676/Documents/CORNELL/multi_datasets', train=False)
    print(len(georgia))
