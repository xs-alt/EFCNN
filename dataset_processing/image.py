...
class Image(object):
    ...
    def crop(self, top_left, bottom_right, resize=None):
        self.img = self.img[top_left[0]:bottom_right[0],
    ...
    def zoom(self, factor):
        ...
        self.img = self.img[sr:self.img.shape[0] -
                            sr, sc: self.img.shape[1] - sc].copy()
        self.img = resize(
            self.img,
            orig_shape,
            mode='symmetric',
...
class DepthImage(Image):
    ...
    def inpaint(self, missing_value=0):
        ...
        # Scale to keep as float, but has to be in bounds -1:1 to keep opencv
        # happy.
